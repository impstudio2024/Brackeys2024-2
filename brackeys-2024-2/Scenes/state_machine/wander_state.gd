...
func move(player: Player, enemy: Enemy):
	...
	var player_in_detection_range: bool = manh(Global.entities.local_to_map(enemy.position), Global.entities.local_to_map(player.position)) < detection_range
	if  player_in_detection_range or state_machine_owner.permanently_angry:
